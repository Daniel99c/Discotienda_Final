...
public class Discotienda
{
	...
	public void generarInformeDiscos () throws FileNotFoundException 
	{
		...
		// Escribir con la pluma en el arhivo

		pluma.println ("reporte de disos");
		pluma.println ("================");
	...
	/**
	 *   Generar un informe de todos los discos (pop)
	 *   todos aquellos que su valor sea > 1000
	 *   @throws FileNotFoundException - cuando no existe la ruta especificada del archivo a leer o escribir
	 */

	public void crearInformeDiscosCostosos ()throws FileNotFoundException
	{
		int generoDelDiscoEncontrado = 0;
		int precioDiscoCostosoEncontrado = 0;
		
		String generoDado = JOptionPane.showInputDialog("Por favor digite un genero de musica ");

		// crear el archivo con la clase FILE

		File archivo = new File ("./data/discosCostosos.txt");

		// crear la pluma para describir el archivo

		PrintWriter pluma = new PrintWriter (archivo);

		// Escribir con la pluma en el archivo 

		pluma.println (" crear informe de discos");
		pluma.println ("========================");

		//Escribir un reporte de discos con nombre,artista y genero 
		// ========================================================

		// realizamos un recorrido total por todos los discos

		for (int i=0; i<discos.size();i++) 
		{
			// recopila la informacion de cada disco

			Disco miDisco = (Disco)discos.get(i);

			// escribir con la pluma informacion del disco que cumla con los requisitos

			if(miDisco.darGenero().equals(generoDado)) {

				generoDelDiscoEncontrado++;
		...
		if (generoDelDiscoEncontrado > 0 && precioDiscoCostosoEncontrado > 0) {
			...
			JOptionPane.showMessageDialog(null, "El disco del genero que ingreso ha sido encontrado :)");

		}else if (generoDelDiscoEncontrado == 0 || precioDiscoCostosoEncontrado == 0) {

			JOptionPane.showMessageDialog(null, "El disco que ingreso no cumple con lo necesario para generar un informe :(");
	...
	{
		try { 
			crearInformeDiscosCostosos();
			return " Reporte generado con exito ";

		}catch (Exception e) {
			return "Error verifique la direccion :(" +e.getMessage();
		}
	}
